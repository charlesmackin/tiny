...
# **********************************************************************
# **********************************************************************
# **** IF ERRORS: SWITCH TO COMPUTE NODE USING INTERACTIVE_SESSION *****
# **********************************************************************
# **********************************************************************
...
# move best chkpt to dccstor for extraction (directory will be large)

param["model_dir"] = 'ares_hwa_models_xneg'   # where to pull from
param["result_dir"] = 'ares_hwa_results_xneg_extracted'
param["model_name"] = 'chkpt_2.pt'

param["result_dir"] = param["result_dir"] + '/' + param["model_name"].replace('.pt', '')

param["dccstor_dir"] = '/dccstor/transformer/charles/extracted_autoencoder_models/' + param["result_dir"]  # where to store extracted model
os.makedirs(param["dccstor_dir"], exist_ok=True)
#######################################################################
...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        print("============== MODEL LOAD ==============")
        # set model path
        model_file_path = param["model_dir"] + "/" + param["model_name"]    # *loading model path using TRAINING chkpt
        ...
        for name, data in model.named_parameters():
            ...
            #extracted_dir = param["model_dir"] + '/' + param["model_name"].replace('.pt', '') + '/extracted_model'
            #activations_dir = param["model_dir"] + '/' + param["model_name"].replace('.pt', '') + '/activations'
            extracted_dir = param["dccstor_dir"] + '/' + param["model_name"].replace('.pt', '') + '/extracted_model'
            activations_dir = param["dccstor_dir"] + '/' + param["model_name"].replace('.pt', '') + '/activations'
        ...
        for dir_name in ['test', 'train']:
            for id_str in machine_id_list:
                print("\n============== BEGIN %s EXTRACTION FOR A MACHINE ID: %s ==============" %(dir_name.upper(), id_str))
                print("dir_name = %s, id_str = %s" %(dir_name, id_str))
                # load test file
                test_files, y_true = com.test_file_list_generator(target_dir, id_str, mode, dir_name=dir_name)
                ...
                for file_idx, file_path in tqdm(enumerate(test_files), total=len(test_files), disable=True):
                    ...
                    try:
                        ...
                        activations_dir2 = activations_dir + '/' + dir_name # make train and test subdir in activations
                        os.makedirs(activations_dir2, exist_ok=True)

                        input_activations_path = activations_dir2 + '/input_' + id_str + '_idx_' + str(file_idx) + '_' + label + '_shape_' + str(pred.shape).replace('(', '').replace(')', '').replace(',', '_').replace(' ', '') + '.csv'
                        output_activations_path = activations_dir2 + '/output_' + id_str + '_idx_' + str(file_idx) + '_' + label + '_shape_' + str(pred.shape).replace('(', '').replace(')', '').replace(',', '_').replace(' ', '') + '.csv'
                        np.savetxt(input_activations_path, data, fmt='%f', delimiter=',')
                        np.savetxt(output_activations_path, pred, fmt='%f', delimiter=',')

                    except Exception as e:
                        com.logger.error("file idx = {} broken!!: {}, {}".format(file_idx, file_path, e))
                ...
                if mode and 'test' in dir_name.lower(): # can only do for test dataset
                    # append AUC and pAUC to lists
                    auc = metrics.roc_auc_score(y_true, y_pred)
                    p_auc = metrics.roc_auc_score(y_true, y_pred, max_fpr=param["max_fpr"]) # max false positive rate
        ...
        if mode and 'test' in dir_name.lower():    # indent here to average for Test dataset but not for Train
            # calculate averages for AUCs and pAUCs
            averaged_performance = np.mean(np.array(performance, dtype=float), axis=0)
            print("Averaged Performance = %f" %averaged_performance)
